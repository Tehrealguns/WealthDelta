      ...
      <div className="mx-auto max-w-4xl flex flex-col md:flex-row gap-12 items-start" style={{ perspective: 1000 }}>
        ...
        <div className="flex-1 w-full max-w-md space-y-3" style={{ perspective: 800 }}>
          {stats.map((item, i) => (
            <motion.div
              key={item.label}
              className="flex items-center justify-between rounded-xl border border-white/[0.08] bg-black/40 px-5 py-4 backdrop-blur-xl"
              variants={cardMorph}
              initial="hidden"
              whileInView="visible"
              viewport={vp}
              custom={i + 3}
            >
              <span className="text-[13px] text-white/30">{item.label}</span>
              <span className={`font-mono text-sm font-medium ${item.accent ? 'text-[#CA8A04]/80' : 'text-white/60'}`}>
                {item.value}
              </span>
            </motion.div>
          ))}
